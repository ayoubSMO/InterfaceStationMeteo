      ...
      <div class="col-md-8">
        <div class="card">
          ...
          <div class="card-body">
              ...
              <button class="btn btn-danger pull-right" onclick="window.location.href='./reclamation'">reclamer</button>
              @if (Auth::user()->etudiant->id_exame == 0)
                @foreach ($examen as $examen)
                        @if ($examen->etat == true)
                        {!!Form::open(['action' => ['App\Http\Controllers\ExamenController@InscrireEtudiant', $examen->id_examen], 'method' => 'PUT', 'class' => 'pull-center'])!!}
                            {{Form::hidden('_method', 'PUT')}}
                            {{Form::submit('Inscrie au Examen', ['class' => 'btn btn-primary'])}}
                        {!!Form::close()!!}
                        @endif
                @endforeach
              @endif
      ...
      <div class="col-md-4">
        <div class="card card-profile">
          ...
          <div class="card-body">
            ...
            <h4 class="card-title" aria-disabled="true">{{Auth::user()->etudiant->Prenom}} {{Auth::user()->etudiant->Nom}}</h4>
